...
const fromSpecifiers = (specifiers = []) => Array.isArray(specifiers) && specifiers.length > 0 ? specifiers.map(specifier => get(specifier, 'local.name')) : [];
...
const getFullImports = traverseOnImports(path => [get(generator__default(path.node), 'code')]);
