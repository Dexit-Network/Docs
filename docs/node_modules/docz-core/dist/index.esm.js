...
const writeDefaultNotFound = async () => {
  const outputPath = join(docz, 'src/pages/404.js'); // If it exists then it would have been created in ensureFiles while copying the theme
...
const ensureFiles = ({
  ...
  const userPagesPath = join(appPath, 'pages');
  const doczPagesPath = join(docz, 'src', 'pages'); // Copy 404 and other possible Gatsby pages

  if (existsSync(userPagesPath)) {
    copySync(userPagesPath, doczPagesPath);
  }
      ...
      try {
        ...
        const {
          settings
              ...
              rest = __rest(modifiedEntry, ["settings"]);
...
function writePropsOnCache(items, config) {
  ...
  outputJSONSync(cacheFilepath, Object.assign(Object.assign({}, cache), newCache));
}
